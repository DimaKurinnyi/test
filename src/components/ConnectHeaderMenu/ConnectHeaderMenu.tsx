...
function ConnectHeaderMenu() {
  ...
  return (
    <div
      ...
      {isSolanaConnected ? (
        <SolanaConnectedButtonContent weight={600} size={17} />
      ) : isEvmConnected ? (
        <EvmConnectedButtonContent weight={600} size={17} />
      ) : (
...
const SolanaConnectedButtonContent = ({ weight, size }: { weight: number; size: number }) => {
  ...
  useEffect(() => {
    const registerSolanaAddress = async () => {
      try {
        const referrerCode =
          new URLSearchParams(window.location.search).get('ref') ?? localStorage.getItem('ref');
...
const SolanaConnectionManagerElement = () => {
  ...
  useEffect(() => {
    setIsConnected(isSolanaConnected);
  }, [isSolanaConnected]);
